        ...
        if smtp.respond_to?(:enable_starttls_auto) 
          unless settings[:openssl_verify_mode]
            ...
            if RUBY_VERSION >= '1.9.0'
              context = Net::SMTP.default_ssl_context
              context.verify_mode = openssl_verify_mode
              smtp.enable_starttls_auto(context) #MINAEV wrong code: smtp.enable_tls(context)
            else
              smtp.enable_starttls_auto(openssl_verify_mode) #MINAEV wrong code: smtp.enable_tls(openssl_verify_mode)
            end
          end
        end
      end
